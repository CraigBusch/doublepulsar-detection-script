...
def check_ip(ip):
    ...
    # Server requires NLA which implant does not support
        ...
        with print_lock:
            print("[-] [" + ip + "] Server requires NLA, which DOUBLEPULSAR does not support")
    ...
    try:
        ...
        with print_lock:
            if len(ping_response) == 288:
                print("[+] [" + ip + "] DOUBLEPULSAR RDP IMPLANT DETECTED!!!")
            else:
                print("[-] [" + ip + "] Status Unknown - Response received but length was ", len(ping_response), " not 288")
        s.close()
    except socket.error as e:
        with print_lock:
            print("[-] [" + ip + "] No presence of DOUBLEPULSAR RDP implant")
